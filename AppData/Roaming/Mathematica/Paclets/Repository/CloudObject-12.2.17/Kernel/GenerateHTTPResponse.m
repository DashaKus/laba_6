...
HTTPResponse /: GenerateHTTPResponse[res:HTTPResponse[_, meta_?AssociationQ, ___]] :=
    Block[
        ...
        makeCachePersistence @ res["Encode"]
    ]
...
responseXML[result_] := 
    With[
        ...
        XMLElement["evaluation-data", 
            KeyValueMap[
                #1 -> ToString[#2] &, KeyTake[result, attrs]
            ], 
            Replace[
                Complement[Keys @ result, attrs], {
                    "OutputLog" :> XMLElement[
                        "OutputLog", {}, 
                    ...
                    "MessagesText" :> XMLElement[
                        "MessagesText", {}, 
                        Map[XMLElement["MessagesText", {}, {#}] &, result["MessagesText"]]
                    ],
                    "MessagesExpressions" :> XMLElement[
                        "MessagesExpressions", {}, 
...
responseFormKey[s_List, rest___] :=
    AssociationMap[responseFormKey[#, rest] &, s]

responseFormKey["StatusCode", evalData_] := 
    evalData["Result"]["StatusCode"]
responseFormKey["Success", evalData_] := 
    evalData["Result"]["StatusCode"] < 400 && evalData["Success"]
responseFormKey[key:"Messages"|"MessagesText"|"MessagesExpressions", evalData_] :=                     
    Join @@ Lookup[
        {evalData, CloudObject`$EvaluationParameters}, 
        key, 
        {}
    ]
responseFormKey[key:"Timing"|"AbsoluteTiming", evalData_] := 
    Plus @@ Lookup[
        {evalData, CloudObject`$EvaluationParameters}, 
        key, 
        0
...
responseFormKey["InputString", evalData_] := 
    evalData["InputString"]
responseFormKey["Result", evalData_] := 
    If[
        evalData["Result"]["BinaryFormatQ"],
        ExportString[evalData["Result"]["Body"], {"Base64", "String"}],
        evalData["Result"]["Body"]
    ]
responseFormKey["ResultMeta", evalData_] := evalData["Result"]["Meta"]

responseFormKey[s_, evalData_] := evalData[s]

responseFormKeyValidator[All] := 
    {
        "StatusCode", "Success", "FailureType", 
        "OutputLog", "Timing", "AbsoluteTiming", 
        "Messages", "MessagesText", "MessagesExpressions", 
        "InputString", "Result", "ResultMeta"
    }
responseFormKeyValidator[s_String] := 
    {s}
responseFormKeyValidator[s_List] := Cases[$ResponseKeys, _String, All]
responseFormKeyValidator[_] :=
    {}
...
makeResponseForm[evalData_] := GenerateHTTPResponse @ Replace[{  
    ...
    responseFormKey[responseFormKeyValidator[$ResponseKeys], evalData]
    }, {
        {"XML", result_} :> 
            HTTPResponse[
        ...
        {None|Null|_Missing|Automatic|"WL"|"STRING"|"TEXT"|"PACKAGE", result_} :> 
            HTTPResponse[
    ...
    With[
        {expr = held, request = Replace[
            req, {
                ...
                any:$httpRequestPattern :> any[{
                    "Scheme", "User", "Domain", "Port", 
                    "PathString", "QueryString", "Fragment", 
                    "MultipartElements", "Headers",
                    "Cookies", "Method"
                }],
...
SetAttributes[
    {AutoRefreshed, Delayed, ExportForm, ResponseForm, HTTPRedirect, HTTPResponse, HTTPErrorResponse,
    GenerateHTTPResponse}
    ,
